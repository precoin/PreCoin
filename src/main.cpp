...
uint256 hashGenesisBlock("0x7f266a4b365e4a468064ba2fcbaa42bb7b071087d4d645847c4d17a4af73bb45");
...
const string strMessageMagic = "PreCoin Signed Message:\n";
...
{
    // PreCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // PreCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1469033698;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1492467;
...
unsigned char pchMessageStart[4] = { 0xd2, 0xca, 0xaf, 0xe1 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PreCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PreCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PreCoinMiner(CWallet *pwallet)
{
    printf("PreCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("PreCoin-miner");
    ...
    try { loop {
        ...
        printf("Running PreCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PreCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PreCoinMiner, pwallet));
}
