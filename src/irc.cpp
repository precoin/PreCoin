...
// Copyright (c) 2011-2012 PreCoin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #PreCoinTEST0\r");
            Send(hSocket, "WHO #PreCoinTEST0\r");
        } else {
            // randomly join #PreCoin00-#PreCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #PreCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #PreCoin%02d\r", channel_number).c_str());
        }
